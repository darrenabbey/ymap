...
function minimizeProject($dir) {
	...
	foreach ($files as $file) {
		if (($file == "bulk.txt") 		or ($file == "colors.txt") 	or ($file == "complete.txt") 	or ($file == "dataBiases.txt") 	or ($file == "dataFormat.txt") 	or ($file == "error.txt")
		or ($file == "figure_options.txt") 	or ($file == "figVer.txt") 	or ($file == "genome.txt") 	or ($file == "index.php") 	or ($file == "name.txt") 	or ($file == "parent.txt")
		or ($file == "ploidy.txt") 		or ($file == "process_log.txt") or ($file == "readStats.txt") 	or ($file == "readStats.txt2") 	or ($file == "salt.txt") 	or ($file == "segmental_aneuploidy.txt")
		or ($file == "showAnnotations.txt") 	or ($file == "update_log.txt") 	or ($file == "warning.txt") 	or ($file == "working.txt")) {
			rename($dir.$file, $temp_dir.$file);
		}
		$file_ext  = substr(strrchr($file, '.'), 1);
		if (($file_ext == "png") or ($file_ext == "gff3") or ($file_ext == "log")) {
			if (!str_contains($file,'ChARM_test') and !str_contains($file,'Rsquared') and !str_contains($file,'CNV_GaussFit') and !str_contains($file,'SNP_GaussFit')) {
				rename($dir.$file, $temp_dir.$file);
			}
		}
