...
trap 'sh queue_end.sh $user $project $main_dir $logName; echo "Something went wrong. project.single_ddRADseq.install_3.sh:$LINENO" > $projectDirectory"error.txt"; exit 1;' ERR;
