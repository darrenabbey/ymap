...
### Final format only has data lines for loci with allelic ratio between ratio cutoffs calculted for the derived dataset.
### Output collumns:
...
with open(logName, "a") as myfile:
	myfile.write("\t\t*==============================================================================*\n");
	myfile.write("\t\t| Log of 'scripts_seqModules/scripts_hapmaps/hapmap.preprocess_derived.py'     |\n");
	myfile.write("\t\t*------------------------------------------------------------------------------*\n");
...
#============================================================================================================
# Load allelic-ratio cutoffs from "allelic_ratios.txt".
#------------------------------------------------------------------------------------------------------------
#	### Chromosome_name, Chromosome_segment, allelic-ratio_cutoffs
#	Ca21chr1_C_albicans_SC5314 1 [32.5,172.5]
#	Ca21chr2_C_albicans_SC5314 1 [52.5,149.5]
#	Ca21chr3_C_albicans_SC5314 1 [22.5,183.5]
#	Ca21chr4_C_albicans_SC5314 1 [18.5,183.5]
#	Ca21chr5_C_albicans_SC5314 1 [20.5,183.5]
#	Ca21chr6_C_albicans_SC5314 1 [32.5,171.5]
#	Ca21chr7_C_albicans_SC5314 1 [17.5,187.5]
#	Ca21chrR_C_albicans_SC5314 1 [31.5,173.5]
# Divide raw cutoff values by 200 to yield percentage.
allelicRatio_inputFile  = main_dir+"users/"+hapmapUser+"/hapmaps/"+hapmap+"/allelic_ratios.txt";
allelicRatioFile        = open(allelicRatio_inputFile,'r');
allelicRatio_data       = allelicRatioFile.readlines();
allelicRatio_dictionary = {};
for line in allelicRatio_data:
	if (len(line) > 0):
		if (line[0] != "#"):
			line_parts = line.strip().split();
			if (len(line_parts) > 0):
				chr_name			  = line_parts[0];
				chr_segment			  = line_parts[1]; # should only ever be '1'.
				chr_cutoffs			  = line_parts[2].lstrip(line_parts[2][0]).rstrip(line_parts[2][-1]).split(','); # strip off brackets, then splits by commas.
				allelicRatio_dictionary[chr_name] = chr_cutoffs;
allelicRatioFile.close();
...
for line in data:
	...
	P_countTot = parentLine[2];   # total count             : 101
	P_refBase  = parentLine[3];   # reference base at bp.	: T
	P_countA   = parentLine[4];   # count of A.		: 100
	P_countT   = parentLine[5];   # count of T.		: 0
	P_countG   = parentLine[6];   # count of G.		: 0
	P_countC   = parentLine[7];   # count of C.		: 1

	if int(float(P_countTot)) > 0:
		...
		totalCount   = float(sum(P_counts));
		P_het        = False;
		P_ratio      = maxAllele1/totalCount;
		left_cutoff  = float(allelicRatio_dictionary[P_chr_name][0])/200; # 0.25 originally.
		right_cutoff = float(allelicRatio_dictionary[P_chr_name][1])/200; # 0.75 originally.

		if (P_ratio > left_cutoff) and (P_ratio < right_cutoff):
			P_het = True;
		else:
			P_het = False;
