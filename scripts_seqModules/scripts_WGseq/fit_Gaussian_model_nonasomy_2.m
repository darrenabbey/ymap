function [p1_a,p1_b,p1_c, p2_a,p2_b,p2_c, p3_a,p3_b,p3_c, p4_a,p4_b,p4_c, p5_a,p5_b,p5_c, p6_a,p6_b,p6_c, p7_a,p7_b,p7_c, p8_a,p8_b,p8_c, p9_a,p9_b,p9_c, p10_a,p10_b,p10_c, Rsquared] = fit_Gaussian_model_nonasomy_2(workingDir, descriptionString, data,locations,init_width,func_type, makeFitFigures)
	...
	p7_ai  = data(round(locations(7)));   p7_bi  = locations(7);   p7_ci  = init_width;
	p8_ai  = data(round(locations(8)));   p8_bi  = locations(8);   p8_ci  = init_width;
	p9_ai  = data(round(locations(9)));   p9_bi  = locations(9);   p9_ci  = init_width;
	p10_ai = data(round(locations(10)));  p10_bi = locations(10);  p10_ci = init_width/4;

	%initial = [p1_ai,p1_ci,p2_ai,p2_ci,p3_ai,p4_ai,p5_ai,p6_ai,p7_ai,p8_ai,p9_ai,p10_ai];
	initial = [p1_ci,p2_ai,p2_ci,p3_ai,p4_ai,p5_ai,p6_ai,p7_ai,p8_ai,p9_ai];
	...
	%p9_a = abs(Estimates(11));	%p9_b = locations(9);	%p9_c = abs(Estimates(4));
	%p10_a = abs(Estimates(12));	%p10_b = locations(10);	%p10_c = abs(Estimates(2));
	...
	p9_a = abs(Estimates(10));								p9_b = locations(9);	p9_c = abs(Estimates(3));
	p10_a = max([data(round(locations(10))) data(round(locations(10))-1)])/max(data);	p10_b = locations(10);	p10_c = abs(Estimates(1));
	...
	p7_fit_R  = p7_a*exp( -0.5*((time7_2 -p7_b )./p7_c ).^2);
	p8_fit_L  = p8_a*exp( -0.5*((time8_1 -p8_b )./p8_c ).^2);
	p8_fit_R  = p8_a*exp( -0.5*((time8_2 -p8_b )./p8_c ).^2);
	p9_fit_L  = p9_a*exp( -0.5*((time9_1 -p9_b )./p9_c ).^2);
	p9_fit_R  = p9_a*exp( -0.5*((time9_2 -p9_b )./p9_c ).^2);
	p10_fit_L = p10_a*exp(-0.5*((time10_1-p10_b)./p10_c).^2);
	p10_fit_R = p10_a*exp(-0.5*((time10_2-p10_b)./p10_c).^2);
...
function sse = fiterror(params,time,data,func_type,locations,show)
	...
	%p9_a = abs(params(11));	%p9_b = locations(9);	%p9_c = abs(params(4));
	%p10_a = abs(params(12));	%p10_b = locations(10);	%p10_c = abs(params(2));
	...
	p7_a = abs(params(8));									p7_b = locations(7);	p7_c = abs(params(3));
	p8_a = abs(params(9));									p8_b = locations(8);	p8_c = abs(params(3));
	p9_a = abs(params(10));									p9_b = locations(9);	p9_c = abs(params(3));
	...
	locs = [p1_b, p2_b, p3_b, p4_b, p5_b, p6_b, p7_b, p8_b, p9_b, p10_b];
	...
	c10_  = p10_c/2 + p10_c*skew_factor10/denoms(10)/2;	p10_c = p10_c*p10_c/c10_;
	...
	%p5_fit_L  = p5_a*exp( -0.5*((time5_1 -p5_b )./p5_c ).^2);
        %p5_fit_R  = p5_a*exp( -0.5*((time5_2 -p5_b )./p5_c /(skew_factor5 /denoms(5)) ).^2);
	...
	%p9_fit_R  = p9_a*exp( -0.5*((time9_2 -p9_b )./p9_c ).^2);
	%p10_fit_L = p10_a*exp(-0.5*((time10_1-p10_b)./p10_c/(skew_factor10/denoms(10)) ).^2);
	%p10_fit_R = p10_a*exp(-0.5*((time10_2-p10_b)./p10_c).^2);
