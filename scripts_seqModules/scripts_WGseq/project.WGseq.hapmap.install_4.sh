...
trap 'bash queue_end.sh $user $project $main_dir $logName "Something went wrong. project.WGseq.hapmap.install_4.sh:$LINENO"; echo "Something went wrong. project.WGseq.hapmap.install_4.sh:$LINENO" > $projectDirectory"error.txt"; exit 1;' ERR;
