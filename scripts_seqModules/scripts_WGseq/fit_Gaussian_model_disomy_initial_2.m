...
function sse = fiterror(params,time,data,func_type,locations,show)
	...
	safe_p1_a = p1_a; if safe_p1_a == 0; safe_p1_a = 0.0001; end;
	p2_c = p2_a/safe_p1_a*p1_c;          % peak width scales with peak height.
	p3_a = p1_a;
	p3_b = locations(3);
	p3_c = p1_c;
	skew_factor = 1;
	...
	denom1 = 100 - abs(100 - p1_b); if denom1 == 0; denom1 = 0.001; end;
	denom3 = 100 - abs(100 - p3_b); if denom3 == 0; denom3 = 0.001; end;

	if (skew_factor/denom1 < 1)
		skew_factor = denom1;
	end;

	c1_  = p1_c/2 + p1_c*skew_factor/denom1/2;
	p1_c = p1_c*p1_c/c1_;
	c3_  = p3_c/2 + p3_c*skew_factor/denom3/2;
	p3_c = p3_c*p3_c/c3_;

	p1_fit_L = p1_a*exp(-0.5*((time1_1-p1_b)./p1_c).^2);
	p1_fit_R = p1_a*exp(-0.5*((time1_2-p1_b)./p1_c/(skew_factor/denom1) ).^2);
	p2_fit   = p2_a*exp(-0.5*((time-p2_b)./p2_c).^2);
	p3_fit_L = p3_a*exp(-0.5*((time3_1-p3_b)./p3_c/(skew_factor/denom3) ).^2);
	p3_fit_R = p3_a*exp(-0.5*((time3_2-p3_b)./p3_c).^2);
