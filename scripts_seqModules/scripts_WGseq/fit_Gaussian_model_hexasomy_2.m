function [p1_a,p1_b,p1_c, p2_a,p2_b,p2_c, p3_a,p3_b,p3_c, p4_a,p4_b,p4_c, p5_a,p5_b,p5_c, p6_a,p6_b,p6_c, p7_a,p7_b,p7_c, Rsquared] = fit_Gaussian_model_hexasomy_2(workingDir, descriptionString, data,locations,init_width,func_type, makeFitFigures)
	...
	%p7_a = abs(Estimates(9));	p7_b = locations(7);	p7_c = abs(Estimates(2));
	...
	p5_a = abs(Estimates(7));							p5_b = locations(5);	p5_c = abs(Estimates(4));
	p6_a = abs(Estimates(8));							p6_b = locations(6);	p6_c = abs(Estimates(4));
	p7_a = max([data(round(locations(9))) data(round(locations(9))-1)])/max(data);	p7_b = locations(7);	p7_c = abs(Estimates(2));

	skew_factor1 = 1; %abs(Estimates(15));
	skew_factor2 = 1; %abs(Estimates(16));
	skew_factor3 = 1; %abs(Estimates(17));
	skew_factor4 = 1;
	skew_factor5 = 1; %2-abs(Estimates(17));
	skew_factor6 = 1; %2-abs(Estimates(16));
	skew_factor7 = 1; %2-abs(Estimates(15));
	...
	if (makeFitFigures)
		...
		saveName = [workingDir 'SNP_GaussFit.' descriptionString '.hexasomy.png'];
...
function sse = fiterror(params,time,data,func_type,locations,show)
	...
	p7_a = max([data(round(locations(9))) data(round(locations(9))-1)])/max(data);	p7_b = locations(7);	p7_c = abs(params(2));

	skew_factor1 = 1; %abs(params(15));
	skew_factor2 = 1; %abs(params(16));
	skew_factor3 = 1; %abs(params(17));
	skew_factor4 = 1;
	skew_factor5 = 1; %2-abs(params(17));
