...
%% process log output.
fprintf(1, 'Chromosones to draw: '+int2str(num_chrs)+'.\n');

first_chr = true;
for chr = 1:num_chrs
	%% process log output.
	fprintf(1, 'Drawing chr '+int2str(chr)+'.\n');

	if (chr_in_use(chr) == 1)
		%% process log output.
		fprintf(1, 'Chr '+int2str(chr)+' in use.\n');
		...
		%% linear : show centromere by pinching the chromosome cartoon outline.
		...
		% note: adding title is done in the end since if placed upper in the code somehow the plot function changes the title position
...
%% Save horizontal aligned genome figure, multiplying height since this is a taller figure.
