...
for chr_to_draw  = 1:length(chr_order)
	...
	if (chr_in_use(chr) == 1)
		...
		if (chr_figReversed(chr) == 0)
			...
			text(-50000/5000/2*3, maxY*3/2,[chr_label{chr} '{\fontsize{stacked_chr_font_size/2}(reversed)}'], 'Rotation',90, 'HorizontalAlignment','center', 'VerticalAlign','bottom', 'Fontsize',stacked_chr_font_size);
		end;
		...
		if (Linear_display == true)
			...
			if (rotate == 0 && chr_size(chr) ~= 0 )
				...
				if (chr_figReversed(chr) == 0)
					...
					text((chr_size(chr)/bases_per_bin)/2,maxY_highTop+0.5,[chr_label{chr} '{\fontsize{linear_chr_font_size/2}(reversed)}'],'Interpreter','none','FontSize',linear_chr_font_size,'Rotation',rotate);
				end;
			end;
