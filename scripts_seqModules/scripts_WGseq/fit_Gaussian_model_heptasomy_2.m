function [p1_a,p1_b,p1_c, p2_a,p2_b,p2_c, p3_a,p3_b,p3_c, p4_a,p4_b,p4_c, p5_a,p5_b,p5_c, p6_a,p6_b,p6_c, p7_a,p7_b,p7_c, p8_a,p8_b,p8_c, Rsquared] = fit_Gaussian_model_heptasomy_2(workingDir, descriptionString, data,locations,init_width,func_type, makeFitFigures)
	...
	p8_ai = max([data(round(locations(8))) data(round(locations(8))-1)])/max(data);		p8_bi = locations(8);	p8_ci = init_width/4;
	skew1 = 0;
	skew2 = 0;
	skew3 = 0;

	initial = [p1_ci,p2_ai,p2_ci,p3_ai,p4_ai,p5_ai,p6_ai,p7_ai,skew1,skew2,skew3];
	...
	p1_a = p1_ai;			p1_b = p1_ai;	p1_c = abs(Estimates(1));
	p2_a = abs(Estimates(2));	p2_b = p2_ai;	p2_c = abs(Estimates(3));	alpha_2 = Estimates(9);
	p3_a = abs(Estimates(4));	p3_b = p3_ai;	p3_c = abs(Estimates(3));	alpha_3 = Estimates(10);
	p4_a = abs(Estimates(5));	p4_b = p4_ai;	p4_c = abs(Estimates(3));	alpha_4 = Estimates(11);
	p5_a = abs(Estimates(6));	p5_b = p5_ai;	p5_c = abs(Estimates(3));	alpha_5 = -Estimates(11);
	p6_a = abs(Estimates(7));	p6_b = p6_ai;	p6_c = abs(Estimates(3));	alpha_6 = -Estimates(10);
	p7_a = abs(Estimates(8));	p7_b = p7_ai;	p7_c = abs(Estimates(3));	alpha_7 = -Estimates(9);
	p8_a = p8_ai;			p8_b = p8_ai;	p8_c = abs(Estimates(1));

	% Minimum variance safety threshold floor bounds
	widths = [p1_c, p2_c, p3_c, p4_c, p5_c, p6_c, p7_c, p8_c];
	widths(widths < 2) = 2;
	p1_c=widths(1); p2_c=widths(2); p3_c=widths(3); p4_c=widths(4); p5_c=widths(5); p6_c=widths(6); p7_c=widths(7); p8_c=widths(8);
	...
	p8_fit = gaussian(time, p8_a, p8_b, p8_c);
	...
	if (makeFitFigures)
		...
		hold on;
		title(['SNP Gaussian model heptasomy; ' descriptionString]);
		if (Rsquared != 0)
		...
		saveName = [workingDir 'SNP_GaussFit.' descriptionString '.heptasomy.png'];
...
function sse = fiterror(params,time,data,func_type,locations)
	...
	p2_a = abs(params(2));									p2_b = locations(2);	p2_c = abs(params(3));	alpha_2 = params(9);
	p3_a = abs(params(4));									p3_b = locations(3);	p3_c = abs(params(3));	alpha_3 = params(10);
	p4_a = abs(params(5));									p4_b = locations(4);	p4_c = abs(params(3));	alpha_4 = params(11);
	p5_a = abs(params(6));									p5_b = locations(5);	p5_c = abs(params(3));	alpha_5 = -params(11);
	p6_a = abs(params(7));									p6_b = locations(6);	p6_c = abs(params(3));	alpha_6 = -params(10);
	p7_a = abs(params(8));									p7_b = locations(7);	p7_c = abs(params(3));	alpha_7 = -params(9);
	p8_a = max([data(round(locations(8))) data(round(locations(8))-1)])/max(data);		p8_b = locations(8);	p8_c = abs(params(1));

	% Minimum variance safety threshold floor bounds.
	widths = [p1_c, p2_c, p3_c, p4_c, p5_c, p6_c, p7_c, p8_c];
        widths(widths < 2) = 2;
	p1_c=widths(1); p2_c=widths(2); p3_c=widths(3); p4_c=widths(4);
        p5_c=widths(5); p6_c=widths(6); p7_c=widths(7); p8_c=widths(8);
