function [p1_a,p1_b,p1_c, p2_a,p2_b,p2_c, p3_a,p3_b,p3_c, p4_a,p4_b,p4_c, p5_a,p5_b,p5_c, p6_a,p6_b,p6_c, p7_a,p7_b,p7_c, p8_a,p8_b,p8_c, Rsquared] = fit_Gaussian_model_heptasomy_2(workingDir, descriptionString, data,locations,init_width,func_type, makeFitFigures)
	...
	p8_ai = data(round(locations(8)));   p8_bi = locations(8);   p8_ci = init_width/4;

	%initial = [p1_ai,p1_ci,p2_ai,p2_ci,p3_ai,p4_ai,p5_ai,p6_ai,p7_ai,p8_ai];
	initial = [p1_ci,p2_ai,p2_ci,p3_ai,p4_ai,p5_ai,p6_ai,p7_ai];
	...
	%p8_a = abs(Estimates(10));	p8_b = locations(8);	p8_c = abs(Estimates(2));
	...
	p8_a = max([data(round(locations(8))) data(round(locations(8))-1)])/max(data);		p8_b = locations(8);    p8_c = abs(Estimates(1));
...
function sse = fiterror(params,time,data,func_type,locations,show)
	...
	%p8_a = abs(params(10));	p8_b = locations(8);	p8_c = abs(params(2));
	...
	p6_a = abs(Estimates(8));								p7_b = locations(7);	p7_c = abs(params(3));
	p7_a = max([data(round(locations(8))) data(round(locations(8))-1)])/max(data);		p8_b = locations(8);	p8_c = abs(params(1));
	...
	locs = [p1_b, p2_b, p3_b, p4_b, p5_b, p6_b, p7_b, p8_b];
	denoms = 100.5 - abs(100.5 - locs);
	denoms(denoms == 0) = 0.001; % Prevents Inf/NaN issues if peak is perfectly centered at 100.5
