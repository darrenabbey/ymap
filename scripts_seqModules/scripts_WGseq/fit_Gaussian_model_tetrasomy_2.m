function [p1_a,p1_b,p1_c, p2_a,p2_b,p2_c, p3_a,p3_b,p3_c, p4_a,p4_b,p4_c, p5_a,p5_b,p5_c, Rsquared] = fit_Gaussian_model_tetrasomy_2(workingDir, descriptionString, data,locations,init_width,func_type, makeFitFigures)
	...
	p5_ai = max([data(round(locations(5))) data(round(locations(5))-1)])/max(data);		p5_bi = locations(5);	p5_ci = init_width/4;
        skew = 0;

	initial = [p1_ci,p2_ai,p2_ci,p3_ai,p4_ai, skew];
	...
	p2_a = abs(Estimates(2));	p2_b = p2_bi;	p2_c = abs(Estimates(3));	alpha_2 = Estimates(6);
	p3_a = abs(Estimates(4));	p3_b = p3_bi;	p3_c = abs(Estimates(3));
	p4_a = abs(Estimates(5));	p4_b = p4_bi;	p4_c = abs(Estimates(3));	alpha_4 = -Estimates(6);
	p5_a = p5_ai;			p5_b = p5_bi;	p5_c = abs(Estimates(1));
