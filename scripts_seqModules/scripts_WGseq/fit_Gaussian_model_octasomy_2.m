function [p1_a,p1_b,p1_c, p2_a,p2_b,p2_c, p3_a,p3_b,p3_c, p4_a,p4_b,p4_c, p5_a,p5_b,p5_c, p6_a,p6_b,p6_c, p7_a,p7_b,p7_c, p8_a,p8_b,p8_c, p9_a,p9_b,p9_c, Rsquared] = fit_Gaussian_model_octasomy_2(workingDir, descriptionString, data,locations,init_width,func_type, makeFitFigures)
	...
	p7_a = abs(Estimates(9));								p7_b = locations(7);	p7_c = abs(Estimates(4));
	p8_a = abs(Estimates(10));								p8_b = locations(8);	p8_c = abs(Estimates(4));
	p9_a = max([data(round(locations(11))) data(round(locations(11))-1)])/max(data);	p9_b = locations(9);	p9_c = abs(Estimates(2));
...
function sse = fiterror(params,time,data,func_type,locations,show)
	...
	p9_a = max([data(round(locations(11))) data(round(locations(11))-1)])/max(data);	p9_b = locations(9);	p9_c = abs(params(2));
	...
	c8_  = p8_c/2 + p8_c*skew_factor8/denoms(7)/2;	p8_c = p8_c*p8_c/c8_;
	c9_  = p9_c/2 + p9_c*skew_factor9/denoms(8)/2;	p9_c = p9_c*p9_c/c9_;
	...
	p7_fit_R = p7_a*exp(-0.5*((time7_2-p7_b)./p7_c).^2);
	p8_fit_L = p8_a*exp(-0.5*((time8_1-p8_b)./p8_c/(skew_factor8/denoms(7)) ).^2);
	p8_fit_R = p8_a*exp(-0.5*((time8_2-p8_b)./p8_c).^2);
	p9_fit_L = p9_a*exp(-0.5*((time9_1-p9_b)./p9_c/(skew_factor9/denoms(8)) ).^2);
	p9_fit_R = p9_a*exp(-0.5*((time9_2-p9_b)./p9_c).^2);
	p1_fit = [p1_fit_L p1_fit_R];
	p2_fit = [p2_fit_L p2_fit_R];
	p3_fit = [p3_fit_L p3_fit_R];
