...
fprintf(['\n** number of chroms in figure = ' num2str(num_chrom)]);
...
if (exist([workingDir FastaName '.seq.mat'],'file') == 0) && (exist([workingDir FastaName '.seqRevCom.mat'],'file') == 0)
    ...
    num_chrom = length(SequenceData);

    %% Initialize the cell arrays needed to contain chromosome sequences.
    sequences         = cell(1,num_chrom);
    rev_com_sequences = cell(1,num_chrom);
...
% ---------------------------------------------------------------------
num_chrom = length(sequences);
fprintf(['\n**\t' num2str(num_chrom) ' chromosomes in genome sequence file.']);
if (exist([workingDir FastaName '.nmerCounts_' num2str(nmer_length) '.mat'],'file') == 0)
    ...
    for chrom = 1:num_chrom
        Query_length = nmer_length;

	fprintf(['\n**\tAnalyzing ' num2str(nmer_length) 'mer repeats in : ' names{chrom}{1}]);
%	disp(names{chrom});

	fprintf(['\n**\t\tseq length = ' num2str(length_nts(chrom))]);

	for Query_offset = 1:(length_nts(chrom)-Query_length+1)
	    Query         = sequences{chrom}(Query_offset:(Query_offset+Query_length-1));
	    rev_com_Query = rev_com_sequences{chrom}((length_nts(chrom)-Query_offset-Query_length+2):(length_nts(chrom)-Query_offset+1));
...
if (exist([workingDir FastaName '.nmerDists_' num2str(nmer_length) '.mat'],'file') == 0)
    ...
    clear repeats;
    repeats = cell(1,num_chrom);
    for chrom = 1:num_chrom
        repeats{chrom}(1:length_nts(chrom)) = 0;
    end;
...
%		disp(names{chrom});
            ...
            if (err1 == false)
                if (~isempty(forward))
                    for j = Query_offset:(Query_offset+Query_length-1)
                        repeats{chrom}(j) = repeats{chrom}(j) + nmer_counts(forward);
                    end;
                end;
            end;
        end;
    ...
    for chrom = 1:num_chrom
        length_nts(chrom) = length(repeats{chrom});
    end;
    clear input_files sequences rev_com_sequences chrom_names i;
end;
...
fprintf(fid,'##\n');
fprintf(fid,'## columns = [chromName, bpCoordinate, repetitivenessScore]\n');
for chrom = 1:num_chrom
	fprintf(['\n**\t' names{chrom}{1}]);
%	disp(names{chrom}{1});
	for bp = 1:length_nts(chrom)
		repetitiveness_value = repeats{chrom}(bp);
		fprintf(fid, names{chrom}{1} );
...
%	s1{chrom} = smooth_gaussian(repeats{chrom},sigma,sigma*2);
%    end;
%    fprintf(']');
%
...
%	for bp = 1:length_nts(chrom)
%	    repetitiveness_value = repeats{chrom}(bp);
%	    fprintf(fid,[names{chrom} '\t' num2str(bp) '\t' num2str(repetitiveness_value) '\n']);
%	end;
%    end;
%    fclose(fid);
%    fprintf('\n**\n');
