...
then
	echo;
	echo "# Command syntax is : 'bash FASTA_GCskew_dictionary.sh [YMAP user name] [YMAP genome name] [YMAP main dir] [YMAP log file] [kmer length]'";
	echo "# ";
...
trap 'bash queue_end.sh $userAccount $genomeName $mainDir $logFile "Something went wrong. FASTA_GCskew_dictionary.sh:$LINENO"; echo "Something went wrong. FASTA_GCskew_dictionary.sh:$LINENO" > $genomeDirectory"error.txt"; exit 1;' ERR;
