...
then
	echo;
	echo -e "# Command syntax is : 'bash FASTA_GCskew_dictionary.sh [YMAP user name] [YMAP genome name] [YMAP main dir] [YMAP log file] [kmer length]'";
	echo -e "# ";
	...
	echo -e "#        [kmer length]      : length of window to examine for GC-skew.";
	echo -e "# ";
	echo -e "# Script will output a figure showing GC skew.";
	echo -e "#     GC skew = (G-C)/(G+C)";
...
trap 'bash queue_end.sh $userAccount $genomeName $mainDir $logFile "Something went wrong. FASTA_GCskew_dictionary.sh:$LINENO"; echo -e "Something went wrong. FASTA_GCskew_dictionary.sh:$LINENO" > $genomeDirectory"error.txt"; exit 1;' ERR;
