...
if (Make_figure_repet)
	...
	fprintf(['\nGenerating repetitiveness figure from ''' genome ''' sequence data.\n']);
		...
		if (chr_in_use(chr) == 1)
			...
			if (Standard_display)
				...
				if (Centromere_format == 0)
					source('../scripts_seqModules/scripts_WGseq/scripts_cartoon_stacked_0.m');
	...
	if (Standard_display)
		...
		system(['chmod 664 ' genomeDir 'fig.repet-map.1.eps']);
		system(['chmod 664 ' genomeDir 'fig.repet-map.1.png']);
	end;
	...
	if (Linear_display)
		...
		%% change permissions of figures.
		system(['chmod 664 ' genomeDir 'fig.repet-map.2.eps']);
		system(['chmod 664 ' genomeDir 'fig.repet-map.2.png']);
	end;
else
