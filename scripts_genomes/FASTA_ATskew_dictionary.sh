...
trap 'sh queue_end.sh $userAccount $genomeName $mainDir $logFile; echo "Something went wrong. FASTA_ATskew_dictionary.sh:$LINENO" > $genomeDirectory"error.txt"; exit 1;' ERR;
