function [centromeres, chromSize, figure_details, annotations, figInfo_ploidy_default] = Load_genome_information_1(genomeDir,genome)
...
discard = fgetl(chromSize_fid);   clear discard; %discard header line.
lines_analyzed = 0;
while not (feof(chromSize_fid))
	line = fgetl(chromSize_fid);
	lines_analyzed = lines_analyzed+1;
	size_chrom = sscanf(line, '%s',1);
...
while not (feof(annotations_fid))
	...
	if (strcmp(line(1),'#') == 0)
		lines_analyzed    = lines_analyzed+1;
		annotations_chrom   = sscanf(line, '%s',1);
...
while not (feof(figInfo_fid))
	...
	if (strlength(line) > 0)
		if (line(1) ~= '#')
			figInfo_chrom    = sscanf(line, '%s',1);
			figInfo_chrom    = str2num(figInfo_chrom);
			figInfo_usechrom = sscanf(line, '%s',2);   for i = 1:size(sscanf(line,'%s',1),2);   figInfo_usechrom(1) = [];   end;
			figInfo_usechrom = str2num(figInfo_usechrom);
			figInfo_label  = sscanf(line, '%s',3);   for i = 1:size(sscanf(line,'%s',2),2);   figInfo_label(1)  = [];   end;

			run = 0;
			if (figInfo_usechrom >  0);                                             run = 1; end;
			if ((figInfo_chrom   == 0) && (strcmp(figInfo_label,'Ploidy') == 1));   run = 2; end;
			...
			if (run == 1)
				...
				figure_details(figInfo_chrom).chrom    = figInfo_chrom;
				figure_details(figInfo_chrom).label  = figInfo_label;
				figure_details(figInfo_chrom).name   = figInfo_name;
				figure_details(figInfo_chrom).usechrom = figInfo_usechrom;
				figure_details(figInfo_chrom).posX   = str2double(figInfo_posX);
				figure_details(figInfo_chrom).posY   = str2double(figInfo_posY);
				figure_details(figInfo_chrom).width  = figInfo_width;
				figure_details(figInfo_chrom).height = str2double(figInfo_height);
			elseif (run == 2)
...
for i = 1:length(figure_details)
	...
	if (figure_details(i).chrom > 0)
		if (strcmp(figure_details(i).width(1),'*') == 0)
			...
			maxchromSize = chromSize(figure_details(i).chrom).size;
		end;
	end;
end;
for i = 1:length(figure_details)
