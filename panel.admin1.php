	...
	if (($admin_logged_in == "true") and isset($_SESSION['logged_on'])) {
		...
		foreach($userFolders as $key=>$userFolder) {
			if (file_exists("users/".$userFolder."/locked.txt")) {
				...
				echo "<font size='2'>".($key_+1).". ".$userFolder."</font></span>\n";
				echo "\t\t</td><td style='text-align:center'>\n";
				echo "\t\t\t<input type='button' value='Approve' style='background-color: yellow;' onclick=\"key = '$key'; $.ajax({url:'admin.approveUser_server.php',type:'post',data:{key:key},success:function(answer){console.log(answer);}}); setTimeout(()=> {location.replace('panel.admin1.php')},500);\">\n";
