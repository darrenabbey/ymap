...
<html lang="en">
	...
	if (!$user == "") {
		...
		?>
					...
					<option value="3">existing hapmap and one diploid</option>
					</select>
					<label for="HapmapSetupOption"> strain(s).</label>
				</td><td>
					Three options to define the SNPs of a hapmap.<br>
					1. One heterozygous diploid and derived strains with homozygous regions.<br>
					2. Two haploids (or homozygous diploids).<br>
					3. Existing hapmap and one derived strain with homozygous regions.
...
<!-- third option. -->
					...
					<div id="hiddenFormSection9" style="display:none">
						Existing hapmap : <select id="parentHapmap" name="parentHapmap"><option>[choose]</option></select>
					</div>
				</td><td valign="top">
					<div id="hiddenFormSection10" style="display:none">
						The existing hapmap you want to adjust for a derived strain background.
					</div>
				</td></tr><tr bgcolor="#CCCCFF"><td valign="top">
					<div id="hiddenFormSection11" style="display:none">
						Diploid strain : <select id="derived" name="derived"><option>[choose]</option></select>
					</div>
				</td><td valign="top">
					<div id="hiddenFormSection12" style="display:none">
						This derived strain will be used to adjust selected hapmap.<br>
						Any homozygous regions will be removed from the hapmap.
					</div>
				</td></tr></table><br>
	<?php
	if (!$user == "") {
...
function UpdateForm() {
	if (document.getElementById("HapmapSetupOption").value == 2) {
		...
		// Pre-existing hapmap and one derived strain with homozygous regions.
		document.getElementById("hiddenFormSection1").style.display   = 'none';
		document.getElementById("hiddenFormSection2").style.display   = 'none';
		document.getElementById("hiddenFormSection3").style.display   = 'none';
		document.getElementById("hiddenFormSection4").style.display   = 'none';
